...
class samplesheet_designer(QWidget):
    ...
    def search_files(self):
        ...
        if self.step == 'variantcalling' or self.step == 'preprocessing':
            files,_ = QFileDialog.getOpenFileNames(self,"Select one or more files",os.path.expanduser("~"),"BAM .bam (*.bam)", options=file_searcher)
        else:
            files,_ = QFileDialog.getOpenFileNames(self,"Select one or more files",os.path.expanduser("~"),"FASTQ .fastq .fq .fastq.gz fq.gz (*.fastq *.fq *.fastq.gz *.fq.gz)", options=file_searcher)
