...
class Analysis_page(QtWidgets.QWidget):
    ...
    def retranslateUi(self):
        ...
        self.start_analysis_pushButton.setText(_translate("Analysis_page", "Start analysis"))
    ...
    def init_panels_combobox(self):
        ...
        panels_cfg = self.configs_folder + '/experiment_list.cfg'
        panels_list = json.loads((open(panels_cfg).read()).encode('utf8'))['list']
        self.panels_comboBox.addItem('-')
        for p in panels_list:
            self.panels_comboBox.addItem(p)
    ...
    def getToolscfg(self):
        ...
        starting_folder = self.configs_folder + '/tools_cfg/'
    ...
    def get_workflow(self):
        ...
        if self.item_shortv_call.checkState(0) == 2: workflow += ["variantcalling"]
        if self.item_cnv_call.checkState(0) == 2: workflow += ["cnvcalling"]
        ...
        if self.item_shortv_ann.checkState(0) == 2: workflow += ["variant_annotation"]
        if self.item_cnv_ann.checkState(0) == 2: workflow += ["cnv__annotation"]
    ...
    def set_step_treewidget(self):
        pipeline_dir = self.configs_folder + '/pipelines/'
        ...
        if os.path.isfile(ss):
            ...
            if 'prealignment' in STEP_workflow:
                self.item_pre_alig.setCheckState(0,2)
            else:
                self.item_pre_alig.setCheckState(0,0)

            if 'alignment' in STEP_workflow:
                self.item_align.setCheckState(0,2)
            else:
                self.item_align.setCheckState(0,0)

            if 'preprocessing' in STEP_workflow:
                self.item_pre_proc.setCheckState(0,2)
            else:
                self.item_pre_proc.setCheckState(0,0)

            if 'postprocessing' in STEP_workflow:
                self.item_post_proc.setCheckState(0,2)
            else:
                self.item_post_proc.setCheckState(0,0)

            if 'postannotation' in STEP_workflow:
                self.item_postann.setCheckState(0,2)
            else:
                self.item_postann.setCheckState(0,0)
    ...
    def start_analysis(self):
        ...
        if self.analysis_list_QListWidget.count() == 0:
            miss_runID = QtWidgets.QMessageBox()  
            miss_runID.setWindowTitle('Empty Queue!')
            miss_runID.setText('Add some analysis to queue!')
            miss_runID.setIcon(QtWidgets.QMessageBox.Critical)
            miss_runID.exec()
            args = ['gnome-terminal','-e']
            helper_args = ['python', '/home/jarvis/print_a.py']
            args += ['bash -c "conda run -n gatk && ' + 'conda list && ' + ' '.join(helper_args) + '"']
        ...
        for row in range(self.analysis_list_QListWidget.count()):
            ...
            runid = item.text()
            panel = self.analysis_dict[runid]['panel']
            pipeline = self.configs_folder + '/pipelines/'+ self.analysis_dict[runid]['pipeline']  + '.pipeline'
            samplesheet = self.analysis_dict[runid]['samplesheet']
            ...
            powercall_main = '/'.join(os.path.dirname(os.path.realpath(__file__)).split('/')[:-1] +['bin/pipeline.py'])

            #success = subprocess.call(args)
            args = ['gnome-terminal','-e']
            
            helper_args = ['python3', powercall_main]
            helper_args += ['--run_id', runid]
            helper_args += ['--panel', panel]
            helper_args += ['--pipeline', pipeline]
            helper_args += ['--workdir', workdir]
            helper_args += ['--workflow', workflow]
            helper_args += ['--samplesheet', samplesheet]
            helper_args += ['--tools_cfg', toolscfg]

            if del_temp == 'True': helper_args += ['--del_temp']
            if paral == 'True': helper_args += ['--parallel']
            
            args += ['bash -c "conda run -n gatk && ' + ' '.join(helper_args) + '"']
            print(' '.join(args))
            success = subprocess.call(helper_args)
            # if not success:
