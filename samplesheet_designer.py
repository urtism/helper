...
class samplesheet_designer(QWidget):
    ...
    def setupUi(self):
        ...
        #self.comboBox.addItems(['prealignment', 'alignment','preprocessing','variant/CNV calling','postprocessing','annotation','postannotation'])
        self.comboBox.addItems(['prealignment', 'alignment','preprocessing','variant/CNV calling'])
        self.retranslateUi()
...
############################ salva le modifiche della table in real-tima ma da errore: item nonetype ###########################
    ...
    #         if self.step == 'Pre-processing' or self.step == 'Variant/CNV calling':
...
#####################################################################################################################################
    ...
    def step_change_action(self):
        ...
        elif self.step == 'preprocessing' or self.step == 'variant/CNV calling':
    ...
    def visualSamples(self):
        ...
        if org == "only cases":
            for sample in self.sample_list:
                ...
                elif self.step == 'Variant/CNV calling':
                    if 'variantcalling' in self.out_samplesheet.keys() or 'cnvcalling' in self.out_samplesheet.keys():
                        if sample in self.out_samplesheet[self.step].keys():
                            ...
                            self.samples["cnvcalling"][sample_name] = {"sample_name":sample_name, "bam":bam}
            ...
            for sample in self.sample_list:
                if self.step == 'prealignment' or self.step == 'alignment':
                    if self.step in self.out_samplesheet.keys():
                        if sample in self.out_samplesheet[step].keys():
                            sample_name = self.out_samplesheet[self.step][sample]['case']["sample_name"]
                            fastq_r1 = self.out_samplesheet[self.step][sample]['case']["fastq_R1"]
                            fastq_r2 = self.out_samplesheet[self.step][sample]['case']["fastq_R2"]
                            fastq_i2 = self.out_samplesheet[self.step][sample]['case']["fastq_I2"]
                    ...
                    if 'variantcalling' in self.out_samplesheet.keys() or 'cnvcalling' in self.out_samplesheet.keys():
                        if sample in self.out_samplesheet[self.step].keys():
                            ...
                            self.tableWidget.setItem(row-1, 1, QTableWidgetItem(bam))
                            self.samples[self.step][sample_name] = {"sample_name":sample_name, "bam":bam}
                            self.samples[self.step]["variantcalling"] = {"sample_name":sample_name, "bam":bam}
                            self.samples[self.step]["cnvcalling"] = {"sample_name":sample_name, "bam":bam}
    ...
    def checkInfileStep(self, filetype):
        ...
        if filetype == 'fastq':
            ...
            if step == 'preprocessing' or step == 'variant/CNV calling':
                return True
            else:
                return False
        ...
        if file_list[0].endswith('fastq') or file_list[0].endswith('fastq.gz') or file_list[0].endswith('fq.gz') or file_list[0].endswith('fq'):
            ...
            if self.checkInfileStep('fastq'):
                ...
                for file in file_list:
                    ...
                    print(re.split('_S'+r'(\d+)', fastq_name))
...
###################################################################################
    ...
    def save_samplesheet(self):
        ...
        try:
            self.out_samplesheet["sample_organization"] = self.org
        except: 
    ...
    def info_from_table(self):
        ...
        for row in range(rows):
            ...
            if self.step == 'Variant/CNV calling':
                bam = self.tableWidget.item(row,1).text()
                self.samples["variantcalling"][sample_id]={"sample_name":sample_id,"bam":bam}
                self.samples["cnvcalling"][sample_id]={"sample_name":sample_id,"bam":bam}
                self.samples["Variant/CNV calling"][sample_id]={"sample_name":sample_id,"bam":bam}
