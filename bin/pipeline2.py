...
def trim_adapters(tool, args, threads, ram, tag, ad1, ad2, fastq1, fastq2, workdir, log, reference_fasta, panel_info, tools_info):
	path = tools_info[tool]['path']
	if tool.upper().startswith('AGENT'):
		agent = t.AGeNT()
		agent.init_tool(path, threads, ram, args)
		return agent.Trimmer(fastq1, fastq2, tag, log, workdir)
...
def fastq_alignment(tool, algorithm, args, threads, ram, sample_name, fastq1, fastq2, workdir, log, reference_fasta, panel_info, tools_info):	
	...
	if tool.upper().startswith('BWA'):
		...
		return bowtie2.align_fastq(algorithm, sample_name, fastq1, fastq2, reference_fasta, log, workdir)
		print('bowtie2')
...
def pipeline_builder(samplesheet, confirmed_workflow, pipeline_configuration, panel_configuration, tools_configuration, runID):
	...
	if 'pre-alignment' in workflow_pipe and 'pre-alignment' in confirmed_workflow:
		samplesheet = pre_alignment(samplesheet, pipeline_configuration['pre-alignment'], reference_fasta, pipeline_configuration, panel_configuration, tools_configuration, runID)
		...
		for sample in samples[sampleID]:
			...
			if fastqR1 != '' and fastqR2 != '':
				...
				if 'trim_adapters' in workflow:
					...
					args = prealignment_info['trim_adapters']['args']
					tag = prealignment_info['trim_adapters']['tag']
					adapter1 = prealignment_info['trim_adapters']['adapter1']
					adapter2 = prealignment_info['trim_adapters']['adapter2']
					fastqR1,fastqR2 = trim_adapters(tool, args, threads, ram, tag, adapter1, adapter2, fastqR1, fastqR2, workdir, log, reference_fasta, panel_configuration, tools_configuration)

				if 'filter_fastq_by_qual' in workflow:
					tool = prealignment_info['filter_fastq_by_qual']['tool']
					args = prealignment_info['filter_fastq_by_qual']['args']
				...
				if 'filter_fastq_by_len' in workflow:
					...
					args = prealignment_info['filter_fastq_by_len']['args']				
				...
				if 'fastq_QC' in workflow:
					...
					args = prealignment_info['fastq_QC']['args']
			...
			samplesheet['alignment'][sampleID][sample_type] = {'sample_name':sample_name, 'fastq_R1':fastqR1, 'fastq_R2':fastqR2, 'fastq_I1':fastqI1, 'fastq_I2':fastqI2}
...
if __name__ == '__main__':
	...
	runID = '20200127_Run_243_Cardio'
	samplesheet = '/home/jarvis/result.json'
	#samplesheet = '/home/jarvis/Scrivania/TEST/pipeline/prova_pipeline.ss'
	confirmed_workflow = ['pre-alignment','alignment','preprocessing','variantcalling','postprocessing']
	#scripts_path = 'maindir/scripts/'
