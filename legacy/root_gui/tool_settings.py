...
class tool_settings(QtWidgets.QWidget):
    ...
    def setupUi(self):
        ...
        self.config = json.loads((open(starting_folder+'/tools.cfg').read()).encode('utf8'))
    ...
    def populate_lists(self):
        ...
        tools_list = self.config["tools_list"]
        self.tool_listWidget.addItems(tools_list)

        databases_list = self.config["databases_list"]
...
## TOOLS
    ...
    def delete_tool(self):
        ...
        if x == QMessageBox.Ok:
            ...
            for tool in tools_to_delete:
                self.config["tools_list"].remove(tool.text())
                self.config.pop(tool.text())
                self.tool_listWidget.takeItem(self.tool_listWidget.row(tool))
                with open(os.path.dirname(os.path.realpath(__file__))+'/configs/tools.cfg', 'w') as cfg:
    ...
    def retrieve_new_tool_infos(self):
        if self.addTool.toolname_lineEdit.text() != "":
            if self.addTool.toolname_lineEdit.text() in self.config["tools_list"]:
                msg = QtWidgets.QMessageBox()
                ...
                print('funzionaa')
                self.config["tools_list"] += [self.addTool.toolname_lineEdit.text()]
    ...
    def retrieve_tool_infos(self):
        if self.addTool.toolname_lineEdit.text() != "":
            ...
            if x == QMessageBox.Ok:
                ...
                index = self.config["tools_list"].index(old_tool)
                self.config["tools_list"][index] = new_tool
...
## TAGS
    ...
    def retrieve_tags(self):
            ...
            if self.addTags.item_shortv_call.checkState(0) == 2: tags += ["variant_calling"]
            if self.addTags.item_cnv_call.checkState(0) == 2: tags += ["cnv_calling"]
...
## DATABASES
    ...
    def retrieve_new_db_infos(self):
        if self.addDB.DBname_lineEdit.text() != "":
            if self.addDB.DBname_lineEdit.text() in self.config["databases_list"]:
                msg = QtWidgets.QMessageBox()
                ...
                self.config["databases_list"] += [self.addDB.DBname_lineEdit.text()]
    ...
    def retrieve_db_infos(self):
        if self.addDB.DBname_lineEdit.text() != "":
            ...
            if x == QMessageBox.Ok:
                ...
                #print(self.config["tools_list"])
                index = self.config["databases_list"].index(old_db)
                self.config["databases_list"][index] = new_db
