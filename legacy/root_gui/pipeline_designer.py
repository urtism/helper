...
class Pipeline_designer(QtWidgets.QWidget):
    ...
    def add_tool(self):
        ...
        for rowindex in range(0,self.step_tableWidget.rowCount()):
            if self.step_tableWidget.verticalHeaderItem(rowindex).text() == 'tool':
                ...
                item = ','.join(self.step_tableWidget.item(rowindex, 0).text().split(',') + [tool.text()])
                self.step_tableWidget.setItem(rowindex, 0, QtWidgets.QTableWidgetItem(item))
    ...
    def save_pipeline(self):
        ...
        path = os.path.dirname(os.path.realpath(__file__))+'/configs/pipelines/' + current_pip_name + '.pipeline'

        print('current_pip_name',current_pip_name)
        if ok:                    
    ...
    def delete_pipeline(self):
        ...
        if current_pip_name == '-' or current_pip_name == 'default':
            ...
            x = msg.exec_()
            if x == QMessageBox.Ok:
                filetodelete = os.path.dirname(os.path.realpath(__file__))+'/configs/pipelines/'+current_pip_name+'.pipeline'
    ...
    #            if step == "Variant Calling":
            if step == "VC SNV & short InDels":
                tool_list = self.tools_cfg["tools"]["variant_calling"]
            ...
            if step == "VC Copy Number Variation":
                tool_list = self.tools_cfg["tools"]["cnv_calling"]
    ...
    def init_variables(self):
        config_default = os.path.dirname(os.path.realpath(__file__))+'/configs/panels_cfg/Tools_test.cfg'
